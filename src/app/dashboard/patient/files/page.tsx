...
import { Alert, Box, Button, Card, CardActionArea, CardContent, IconButton, MenuItem, Select, Stack, TextField, Typography } from "@mui/material";
    ...
    <Box sx={{ p: 2 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
        ...
        <Stack direction="row" spacing={1} alignItems="center" sx={{ flexWrap: "wrap", rowGap: 1 }}>
          ...
          <Button disabled={!file || uploading} onClick={doUpload} variant="contained">{uploading ? `Uploading ${progress|0}%` : "Upload"}</Button>
        </Stack>
      </Stack>
      {err && <Alert severity="error" sx={{ mb: 2 }}>{err}</Alert>}
      <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1, flexWrap: "wrap", rowGap: 1 }}>
        <Typography variant="body2">Filter:</Typography>
        <TextField size="small" type="number" label="Min KB" value={minKB} onChange={(e) => setMinKB(e.target.value === "" ? "" : Number(e.target.value))} sx={{ width: 120 }} />
        <TextField size="small" type="number" label="Max KB" value={maxKB} onChange={(e) => setMaxKB(e.target.value === "" ? "" : Number(e.target.value))} sx={{ width: 120 }} />
        <Button size="small" onClick={() => { setMinKB(""); setMaxKB(""); }}>Reset</Button>
      </Stack>
            ...
            <Card variant="outlined">
              <CardActionArea href={it.url} target="_blank">
                <Box sx={{ position: "relative", height: 160 }}>
                  {/^(https?:)?\/\//.test(it.url) ? (
                    <Image src={it.url} alt={it.id} fill sizes="(max-width: 600px) 100vw, 33vw" style={{ objectFit: "cover" }} />
                  ) : (
                    <img src={it.url} alt={it.id} height={160} style={{ width: "100%", objectFit: "cover" }} />
                  )}
                </Box>
                <CardContent>
