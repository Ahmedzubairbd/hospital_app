...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!primaryBase) {
      return NextResponse.json(
        { error: "SMS endpoint not configured", toUser },
        { status: 500 },
      );
    }

    // Try primary host first, then fallback IP
    const first = await hitSendText(primaryBase, toUser, message, {
      apiKey,
      secretKey,
      senderId,
    });
    ...
    if (FALLBACK && FALLBACK !== primaryBase) {
      ...
      if (second.ok)
        return NextResponse.json({ toUser, response: second.body });
      return NextResponse.json(
        {
          toUser,
