...
const patchSchema = z.object({ title: z.string().min(1).optional(), body: z.string().min(1).optional(), excerpt: z.string().optional(), published: z.boolean().optional(), slug: z.string().min(1).optional() });
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const page = await (prisma as any).cmsPage.update({ where: { id }, data: input });
    return NextResponse.json(page);
  } catch (e: unknown) {
